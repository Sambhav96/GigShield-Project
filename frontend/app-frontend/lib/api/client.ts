...
function normalizeApiErrorMessage(payload: any, fallback: string): string {
    ...
    const flattened = detail
      .map((item: any) => {
        ...
        if (item && typeof item === "object") {
          ...
          if (Array.isArray(item.loc) && item.loc.length > 0 && typeof item.msg === "string") {
...
export async function fetchApi<T>(endpoint: string, options?: RequestInit): Promise<ApiResponse<T>> {
  try {
    ...
    if (!response.ok) {
      ...
      const messageFromJson = normalizeApiErrorMessage(
        payload,
        parsed.errorMessage || response.statusText || "Request failed"
      );
      return {
        data: null,
        error: {
