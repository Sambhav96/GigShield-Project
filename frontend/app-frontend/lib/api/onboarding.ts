...
export const onboardingService = {
  ...
  /**
   * PATCH /api/v1/policies/{policy_id}/plan
   * Body: { plan }
   */
  updatePolicyPlan: async (
    policyId: string,
    plan: string
  ): Promise<ApiResponse<PolicyData>> => {
    const res = await fetchApi<PolicyResponse>(riderPath(`/policies/${policyId}/plan`), {
      method: "PATCH",
    ...
    const latest = await onboardingService.getPolicy();
