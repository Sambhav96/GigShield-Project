...
const PARAM_DEFS: Record<string, { label: string; min: number; max: number; step: number; unit: string; description: string }> = {
  ...
  risk_profile_high_mult:  { label: "High Risk Profile Multiplier", min: 1.05, max: 1.30, step: 0.05, unit: "×", description: "Multiplier applied for high risk riders." },
  discount_per_clean_week: { label: "Discount / Clean Week", min: 0.01, max: 0.10, step: 0.01, unit: "", description: "Premium discount % per clean week." },
  max_discount_weeks:      { label: "Max Discount Weeks", min: 2.0, max: 8.0, step: 1.0, unit: "weeks", description: "Maximum weeks a discount can accumulate over." },
  confidence_band_1_factor:{ label: "Confidence Band 1 Factor", min: 0.80, max: 1.00, step: 0.05, unit: "×", description: "Multiplier for >=0.85 oracle score." },
  confidence_band_2_factor:{ label: "Confidence Band 2 Factor", min: 0.70, max: 0.95, step: 0.05, unit: "×", description: "Multiplier for 0.75-0.85 oracle score." },
  daily_soft_limit_divisor:{ label: "Daily Soft Limit Divisor", min: 3.0, max: 6.0, step: 0.5, unit: "", description: "Weekly cap divisor for soft limit logic." },
  vov_reward_zone_cert:    { label: "VOV Zone Cert Reward", min: 10.0, max: 50.0, step: 5.0, unit: "₹", description: "Reward for region certification." },
};
    ...
    <AdminInstantLoad>
      <div className="space-y-6 pb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-['Inter'] font-black text-white tracking-tight">Actuarial</h1>
