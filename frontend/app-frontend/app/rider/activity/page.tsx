  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const mapped = activityList.map((a: any, idx: number) => ({
        ...
        title: cleanText(a.title, "Activity"),
        subtitle: cleanText(a.subtitle || a.description, "Update received"),
        val: cleanText(a.value || a.status, "Logged"),
      })) as ActivityItem[];
      ...
      <div className="space-y-6 pt-2">
        ...
        >
          ...
          {!isLoading && visualized.length === 0 && (
            <motion.div
              variants={{ hidden: { opacity: 0, x: -10 }, visible: { opacity: 1, x: 0 } }}
              className="flex items-center p-4 bg-white/[0.02] border border-white/5 rounded-xl"
            >
              <div className="flex items-center gap-4">
