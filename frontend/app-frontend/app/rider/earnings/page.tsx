      ...
      <motion.div initial="hidden" animate="visible" transition={{ staggerChildren: 0.1 }} className="space-y-6">
        ...
        <motion.div variants={itemVariants} className="text-center pt-4">
          ...
          <button
            ...
            className="bg-[var(--color-rider-primary)] text-[#002635] px-6 py-3.5 rounded-full font-bold font-['Space_Grotesk'] text-base sm:text-lg shadow-[0_0_20px_rgba(84,199,252,0.3)] active:scale-95 transition-transform w-full"
          >
            {showHistory ? "Hide Settlement History" : "View Settlement History"}
          </button>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="pt-4 space-y-4">
          ...
          {settlements.length === 0 ? (
            ...
            settlements.map((p, idx) => {
              ...
              return (
                <div key={payoutId} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between bg-white/[0.02] border border-white/5 rounded-xl p-4">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#a8abb3] break-words">{dateLabel}</p>
                    </div>
                  </div>
                  <p className="text-[var(--color-rider-secondary)] font-['Space_Grotesk'] font-bold sm:text-right">{sign}₹{Math.abs(amount).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
              );
            })
          )}
        ...
        {showHistory && (
          <motion.div
            ...
            className="rounded-2xl border border-white/10 bg-[#0c141d] p-4 sm:p-5"
          >
            <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#a8abb3]">Settlement History</h3>

            {settlements.length === 0 ? (
              <p className="text-sm text-white/60">No settlement history available yet.</p>
            ) : (
                ...
                <table className="min-w-[560px] w-full text-left">
                  <thead>
                    <tr className="border-b border-white/10 text-[10px] uppercase tracking-widest text-white/50">
                      <th className="py-2 pr-4 font-['Manrope']">Date</th>
                      <th className="py-2 pr-4 font-['Manrope']">Reference</th>
                      <th className="py-2 pr-4 font-['Manrope']">Status</th>
                      <th className="py-2 text-right font-['Manrope']">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {settlements.map((p, idx) => {
                      const dt = p.released_at || p.initiated_at;
                      const payoutId = String(p?.id || `history-${idx}`);
                      const title = p.reference || p.claim_reference || `Claim ${payoutId.slice(0, 8)}`;
                      const amount = Number(p?.actual_payout ?? p?.amount ?? 0);
                      const status = String((p as any)?.status || p?.razorpay_status || "processed");

                      return (
                        <tr key={payoutId} className="border-b border-white/5 text-sm">
                          ...
                          <td className="py-3 pr-4 text-white break-words">{title}</td>
                          <td className="py-3 pr-4 text-white/70 capitalize">{status.replaceAll("_", " ")}</td>
                          <td className="py-3 text-right font-bold text-[var(--color-rider-secondary)] whitespace-nowrap">
