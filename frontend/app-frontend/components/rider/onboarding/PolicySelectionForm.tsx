...
export function PolicySelectionForm() {
  ...
  const preferredPlan = (searchParams.get("plan") || "").toLowerCase();
  ...
  const fetchAllQuotes = async (targetHubId: string) => {
    ...
    if (["basic", "standard", "pro"].includes(preferredPlan) && updated[preferredPlan]) {
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      if (res.error || !res.data) {
        ...
        // If rider already has active policy, upgrade/downgrade plan if needed.
        if (msg.toLowerCase().includes("already has an active policy")) {
          const existingPolicy = await onboardingService.getPolicy();
          if (existingPolicy.error || !existingPolicy.data?.id) {
            throw new Error("Unable to read current policy for upgrade.");
          }

          const currentPlan = String(existingPolicy.data.plan_name || "").trim().toLowerCase();
          if (currentPlan !== selectedPlan.toLowerCase()) {
            const upgradeRes = await onboardingService.updatePolicyPlan(existingPolicy.data.id, selectedPlan);
            if (upgradeRes.error || !upgradeRes.data) {
              throw new Error(normalizeErrorMessage(upgradeRes.error?.message));
            }
          }
