...
def process_claim_payout_task(self, claim_id: str, payout_type: str = "initial"):
    ...
    try:
        ...
        if payout_type == "initial":
            if trigger.get("is_synthetic") and trigger.get("raw_api_data") and "synthetic_duration" in trigger["raw_api_data"]:
                dur_hrs = float(trigger["raw_api_data"]["synthetic_duration"])
            else:
                dur_hrs = get_min_duration(trigger["trigger_type"])
        else:
            dur_hrs = 0.5
        ...
        if claim.get("status") == "manual_adjusted" and claim.get("admin_custom_amount"):
            final_payout = float(claim["admin_custom_amount"])
        elif mode == "stressed":
            final_payout *= 0.90
        ...
        # Allow 'auto_cleared' because oracle_worker sets it before queueing. Preserve existing statuses.
        cur.execute("UPDATE claims SET status=(CASE WHEN status='evaluating' THEN 'auto_cleared' ELSE status END),actual_payout=%s WHERE id=%s AND status NOT IN ('paid','rejected') RETURNING id", (actual_payout, claim_id))
        if not cur.fetchone():
        ...
        try:
            ...
            try:
                conn2, cur2 = _conn()
                cur2.execute("UPDATE payouts SET razorpay_ref=%s,razorpay_status='success',reconciled_at=NOW() WHERE idempotency_key=%s", (rz.get("id"), idem_key))
                cur2.execute("UPDATE claims SET status='paid',paid_at=NOW() WHERE id=%s", (claim_id,))
...
def process_provisional_payout(claim_id: str, full_amount: float):
    ...
    if faid and cur.fetchone():
        try:
            ...
            cur.execute("UPDATE payouts SET razorpay_ref=%s,razorpay_status='success',reconciled_at=NOW() WHERE idempotency_key=%s", (rz.get("id"), ikey))
...
def drain_recovery_queue():
    ...
    for idem_key in held:
        ...
        try:
            ...
            cur.execute("UPDATE payouts SET razorpay_ref=%s,razorpay_status='success',reconciled_at=NOW() WHERE idempotency_key=%s", (rz.get("id"), idem_key))
            redis.zrem("payout_recovery_queue", idem_key)
        except Exception as exc:
