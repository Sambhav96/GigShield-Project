    ...
    try:
        ...
        for pol in policies:
            try:
                ...
                # ── DEMO: synthetic triggers bypass fraud checks ──────────────────
                is_synthetic = bool(trigger.get("is_synthetic", False))
                if is_synthetic:
                    # Skip all fraud detection for God Mode demo triggers
                    fraud_score = 0.05
                    disposition = "auto_cleared"
                    presence_conf = 1.0
                    factors = {"f1_gps": True, "f2_session": True, "f3_platform": True, "f3_platform_unavailable": False}
                else:
                    # Fraud evaluation
                    cur.execute("""
