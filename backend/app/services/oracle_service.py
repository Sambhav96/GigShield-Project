...
CODE-05 FIX: platform_down uses health check score directly (not weather=0.0)
GAP-19 FIX:  Weatherstack (heat primary), Earth Engine (flood primary)
"""
...
CACHE_TTL = {"owm_weather": 900, "waqi_aqi": 1800, "earth_engine": 7200, "weatherstack": 7200}
...
def _fetch_with_fallback(primary_fn, fallback_fn, ck, ttl, redis, *args, **kwargs):
    ...
    if fallback_fn:
        try:
            ...
            if r:
                r["_ts"] = time.time()
                redis.set(ck, json.dumps(r, default=str), ex=ttl)
                log.info("oracle_fallback_used", cache_key=ck)
    ...
    if cached_raw:
        try:
            c = json.loads(cached_raw)
            log.info("oracle_stale_cache_used", cache_key=ck)
            return c, "stale_cache", 0.15
        except Exception: pass
...
def compute_oracle_score(
    ...
    if trigger_type == "rain":
        ...
        # HERE Traffic dependency REMOVED
