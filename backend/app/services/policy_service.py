...
async def pause_policy(conn, policy_id, rider_id, reason: str | None = None) -> dict:
    ...
    await conn.execute(
        ...
        uuid.UUID(policy_id), reason or "rider_initiated",
    )
...
async def change_policy_plan(conn, policy_id, rider_id, new_plan: str) -> dict:
    policy = await conn.fetchrow(
        "SELECT * FROM policies WHERE id=$1 AND rider_id=$2",
        uuid.UUID(policy_id), uuid.UUID(rider_id),
    )
    ...
    if str(policy["status"]) in ("cancelled", "lapsed"):
        raise ValidationError(f"Policy is {policy['status']}, cannot change plan")

    current_plan = str(policy["plan"])
    if current_plan == new_plan:
        return {
            ...
            "plan_cap_multiplier": int(policy["plan_cap_multiplier"]),
        }
    ...
    quote = await get_premium_quote(conn, rider_id, new_plan, hub_id)
    coverage_pct = get_plan_coverage(new_plan, str(rider["tier"]))
    plan_cap_mult = PLAN_CAP_MULTIPLIER[new_plan]
    ...
    log.info("policy_plan_changed", policy_id=policy_id, rider_id=rider_id, from_plan=current_plan, to_plan=new_plan)
    return {
        "policy_id": policy_id,
        "plan": new_plan,
        "status": str(policy["status"]),
        "weekly_premium": float(quote["p_final"]),
        "coverage_pct": coverage_pct,
        "plan_cap_multiplier": plan_cap_mult,
    }
