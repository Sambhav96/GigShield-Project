...
async def live_dashboard(
    ...
    # Respect the Global Kill Switch for UI display (unless it's a synthetic God Mode trigger)
    kill_switch = await conn.fetchval("SELECT value FROM system_config WHERE key='global_kill_switch'")
    if kill_switch in ("full", "triggers_only"):
        if active_trigger and not trigger.get("is_synthetic"):
            active_trigger = None
