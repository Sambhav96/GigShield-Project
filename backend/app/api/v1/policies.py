...
from app.models.policy import PolicyCreate, PolicyStatusUpdate, PolicyPlanUpdate
from app.services.policy_service import (
    enroll_policy, pause_policy, resume_policy, cancel_policy, change_policy_plan
)
...
async def get_my_policy(
    ...
    from app.repositories.policy_repo import get_current_policy_for_rider
    policy = await get_current_policy_for_rider(conn, str(rider["id"]))
    if not policy:
        raise HTTPException(status_code=404, detail="No active or paused policy")
...
@router.patch("/{policy_id}/plan")
async def update_policy_plan(
    policy_id: UUID,
    body: PolicyPlanUpdate,
    rider: dict = Depends(get_current_rider),
    conn: asyncpg.Connection = Depends(get_db),
):
    return await change_policy_plan(conn, str(policy_id), str(rider["id"]), body.plan.value)
