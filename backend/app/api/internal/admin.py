...
# ── Debug: code version probe (no auth needed) ────────────────────────────────
@router.get("/debug/version")
async def debug_version():
    return {"version": "v3-try-except-loaded", "ok": True}
...
async def admin_dashboard(admin: dict = Depends(get_current_admin), conn: asyncpg.Connection = Depends(get_db)):
    ...
    liquidity = await conn.fetchrow("SELECT * FROM liquidity_snapshots ORDER BY created_at DESC LIMIT 1")
    kill_switch = await conn.fetchval("SELECT value FROM system_config WHERE key='global_kill_switch'")

    if isinstance(active_policies, Exception):
        active_policies = 0
    if isinstance(payouts_today, Exception) or payouts_today is None:
        payouts_today = {"cnt": 0, "total": 0}
    if isinstance(pending_claims, Exception):
        pending_claims = 0
    if isinstance(active_triggers, Exception):
        active_triggers = 0
    if isinstance(lr, Exception) or lr is None:
        lr = {"pay": 0, "prem": 0}
    if isinstance(liquidity, Exception):
        liquidity = None
    if isinstance(kill_switch, Exception):
        kill_switch = "off"
    ...
    # API budget and ML metrics are sync helpers; run in worker threads to avoid blocking loop.
    async def _load_budget_status():
        try:
            from app.core.api_budget import get_budget_status
            from app.core.redis_client import get_sync_redis
            return await asyncio.to_thread(get_budget_status, get_sync_redis())
        except Exception:
            return {}

    async def _load_ml_metrics():
        try:
            from app.ml.vulnerability_model import get_model_metrics
            return await asyncio.to_thread(get_model_metrics)
        except Exception:
            return {}

    budget_status, ml_metrics = await asyncio.gather(_load_budget_status(), _load_ml_metrics())
    ...
    import json
    with open("debug_admin.json", "w") as f:
...
async def set_kill_switch(body: dict, admin: dict = Depends(get_current_admin), conn: asyncpg.Connection = Depends(get_db)):
    ...
    # Also cancel all active triggers if engaging the kill switch
    # Note: State machine prevents 'resolving' -> 'cancelled', so we set 'resolving' -> 'resolved'
    if val != "off":
        await conn.execute("UPDATE trigger_events SET status='cancelled', resolved_at=NOW() WHERE status = 'active'")
        await conn.execute("UPDATE trigger_events SET status='resolved', resolved_at=NOW() WHERE status = 'resolving'")
...
async def run_stress_test(body: dict, admin: dict = Depends(get_current_admin), conn: asyncpg.Connection = Depends(get_db)):
    ...
    # Get total riders in the city (regardless of policy status) for population base
    total_city_riders = await conn.fetchval(
        "SELECT COUNT(*) FROM riders r JOIN hubs h ON r.hub_id=h.id WHERE (h.city=$1 OR $1='all')",
        city,
    ) or 0
    
    # DEMO-READY: If no riders in DB, assume a standard city fleet size of 1200
    if total_city_riders == 0:
        total_city_riders = 1200

    affected = int(total_city_riders * pct_affected)
    cov_pct  = get_plan_coverage(plan, tier)
    
    # Use a stress-specific intensity multiplier (Oracle score 0.90 equivalent)
    stress_score = 0.90 

    payout_per_rider = avg_income * cov_pct * (duration_hrs / 8) * stress_score
    total_payout     = payout_per_rider * affected

    from app.utils.mu_table import PLAN_BASE_PREMIUM, get_city_multiplier
    
    # Calculate potential weekly revenue for the entire city fleet on this plan
    # Sync: Use PLAN_BASE_PREMIUM * city_multiplier for realism
    base_val = PLAN_BASE_PREMIUM.get(plan, 49.0)
    city_mult = get_city_multiplier(city)
    weekly_premium = base_val * city_mult
    
    city_weekly_revenue = total_city_riders * weekly_premium

    sim_loss_ratio = total_payout / max(city_weekly_revenue, 1)
    ...
    return {
        "disclaimer": "STRESS SIMULATION — Models city-wide impact on reserves.",
        "inputs": body,
        "results": {
            "city_population": total_city_riders,
            ...
            "city_weekly_revenue": round(city_weekly_revenue, 2),
...
async def god_mode_trigger(
    ...
    body: { trigger_type: str, hub_id: str, oracle_score: float, duration: float }
    """
    try:
        trigger_type = body.get("trigger_type", "rain")
        ...
        if trigger_type not in ("rain", "flood", "heat", "aqi", "bandh", "platform_down"):
            return {"status": "error", "error": f"Invalid trigger_type: {trigger_type}"}

        hub_id_input = hub_id.strip() if hub_id else None

        hubs_to_trigger = []
        if hub_id_input:
            try:
                hub = await conn.fetchrow("SELECT * FROM hubs WHERE id = $1::uuid", hub_id_input)
                if hub:
                    hubs_to_trigger.append(hub)
                else:
                    return {"status": "error", "error": f"Hub '{hub_id_input}' not found. Use SELECT id, name FROM hubs; to see valid IDs."}
            except Exception as e:
                return {"status": "error", "error": f"Invalid Hub ID format: {str(e)}"}
        else:
            hubs_to_trigger = await conn.fetch("SELECT * FROM hubs")
            if not hubs_to_trigger:
                return {"status": "error", "error": "No hubs found in system. Please seed the database first."}

        # Lazy import to avoid import-time crash inside async context
        try:
            from app.workers.oracle_worker import initiate_claims_for_hex
            celery_available = True
        except Exception as ce:
            log.warning("oracle_worker_import_failed", error=str(ce))
            celery_available = False

        trigger_ids = []
        for hub in hubs_to_trigger:
            try:
                trigger_id = await conn.fetchval(
                    """
                    INSERT INTO trigger_events (
                        trigger_type, h3_index, hub_id, oracle_score,
                        consensus_score, status, is_synthetic, triggered_at, raw_api_data
                    ) VALUES ($1, $2, $3, $4, $4, 'active', true, NOW(), $5::jsonb)
                    RETURNING id
                    """,
                    trigger_type, hub["h3_index_res9"], hub["id"], oracle_score,
                    json.dumps({"synthetic_duration": duration})
                )
                ...
                if celery_available:
                    try:
                        ...
                        log.warning("god_mode_claims_queue_failed", h3_index=hub["h3_index_res9"], error=str(exc))
            except Exception as hub_exc:
                log.error("god_mode_hub_trigger_failed", hub_id=str(hub["id"]), error=str(hub_exc))

        if not trigger_ids:
            return {"status": "error", "error": "No triggers were created. Check the backend logs for details."}

        try:
            await conn.execute(
                "INSERT INTO admin_audit_log(admin_id,action,entity_type,entity_id,diff,performed_at) "
                "VALUES($1,'god_mode_trigger','trigger_event',$2,$3::jsonb,NOW())",
                admin["id"], "multiple" if len(trigger_ids) > 1 else str(trigger_ids[0]),
                json.dumps({
                    "trigger_type": trigger_type,
                    "oracle_score": oracle_score,
                    "hub_count": len(hubs_to_trigger),
                    "all_hubs": not hub_id_input,
                })
            )
        except Exception as audit_exc:
            log.warning("god_mode_audit_log_failed", error=str(audit_exc))

        return {
            "status": "trigger_created",
            "trigger_id": trigger_ids[0] if trigger_ids else "",
            "trigger_type": trigger_type,
            "hub_name": "ALL HUBS" if not hub_id_input else hubs_to_trigger[0]["name"],
            "hub_id": "all" if not hub_id_input else str(hubs_to_trigger[0]["id"]),
            "h3_index": "multiple" if not hub_id_input else hubs_to_trigger[0]["h3_index_res9"],
            "oracle_score": oracle_score,
            "is_synthetic": True,
            "claims_queued": celery_available,
            "hubs_triggered": len(trigger_ids),
            "message": f"Trigger fired for {len(trigger_ids)} hub(s). Claims queued: {celery_available}.",
        }

    except Exception as top_exc:
        log.error("god_mode_trigger_unhandled", error=str(top_exc))
        return {"status": "error", "error": f"Unhandled server error: {str(top_exc)}"}
