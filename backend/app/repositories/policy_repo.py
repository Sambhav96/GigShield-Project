...
async def get_current_policy_for_rider(
    conn: asyncpg.Connection, rider_id: str
) -> Optional[dict]:
    """Return latest rider policy that is still controllable in rider Shield UI."""
    row = await conn.fetchrow(
        """
        SELECT * FROM policies
        WHERE rider_id = $1 AND status IN ('active', 'paused')
